...
#
df = pd.read_csv('D:\Online COurses\Python\Data\Module5Clustering\Module5Clustering\Datasets/CDR.csv',header = 0)
...
#
b = user1[(user1.DOW == 'Sat') |(user1.DOW == 'Sun')]
...
#
c = b[(b.CallTime<"06:00:00") | (b.CallTime>"22:00:00")]
len(c)
user1 = c
#
...
def kmeans_loc(user):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(centroids[:,0],centroids[:,1], c='b', marker='o', alpha=0.8)
    ax.set_title('kmeans center for'+ 'user'+'i+1' + 'K=' + str(1))
    ax.set_xlabel('TowerLon')
    ax.set_ylabel('TowerLat')
    print('User'+ str(i+1) + str(centroids))
...
for i in range(0,len(a)):
    print('user'+str(i+1))
    user = df[df.In == a[i]]
    b = user[(user.DOW == 'Sat') |(user.DOW == 'Sun')]
    c = b[(b.CallTime<"06:00:00") | (b.CallTime>"22:00:00")]
    user = c
    kmeans_loc(user)
